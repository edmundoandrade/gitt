...
(* Apply Method of Solution to transform the PDE in ODE *)
  ...
  eqtrfgov=eqtrfgov /.
    defIntegral[a_,vars_]-defIntegral[b_,vars_]:>Integrate[a-b,vars];
  eqtrfgov=eqtrfgov /.
    Integrate:>defIntegral;
...
(* Simplify by using the boundary conditions *)
  ...
  simp=exptrf;
  tmp=Solve[Flatten[{h[t]==simp,eqicbc,eqauxbc}],h[t]];
...
(* Return inversion formula and transformed ODE *)
  ...
  {rlinv,eqtrfgov,eqtrficbc,unknownsT}
...
Module[{glbsol,eigenc,fs,veqn,ret,vsi},
  ...
  eigenc=Reduce[Join[eigenc,{v!=0,C[1]!=0||C[2]!=0}],{C[1],C[2]}];
  ret=$Failed;
