...
ClearAll[changeFunction,getOperator,isLinear,getLinearOperator,defIntegral,defIntegralFromIndefinite];
...
defIntegralFromIndefinite::usage =
"defIntegralFromIndefinite[expr,{x,xi,xf}] gives the definite integral of expr with
respect to x on domain [xi,xf] after trying solve the indefinite integral."
...
getOperator[expr_,f_[vs___Symbol]] :=
  Module[{fdep,g},
	...
	(Evaluate[changeFunction[fdep,f[vs],Hold[g[vs]]]])& //.
	{Hold[g[vs]]->#,Hold[r_ReplaceAll]->r}
  ]
...
defIntegral[f_, s:{x_Symbol,xi_,xf_}] :=
  Module[{it=defIntegralFromIndefinite[f,s]},
	it /; Head[it]=!=defIntegralFromIndefinite
  ]

defIntegralFromIndefinite[f_, s:{x_Symbol,xi_,xf_}] :=
  Module[{it=Integrate[f,x],is,iv},
	(is = Select[1+it,FreeQ[#,Integrate]&];
	 iv = Select[1+it,!FreeQ[#,Integrate]&] /. Integrate[g_,x]:>defIntegral[g,s];
	 (is/.x->xf)-(is/.x->xi)+iv
	) /; Head[it]=!=Integrate
  ]
